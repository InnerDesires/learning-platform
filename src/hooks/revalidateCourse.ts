...
// Busts every ISR-cached surface a course appears on (step and quiz pages render per
// request and need none). Scheduled publishes run outside a request, where
// revalidatePath throws; a missed bust falls back to the time window, so never fail the save.
