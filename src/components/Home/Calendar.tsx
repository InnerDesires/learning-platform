...
export function CalendarSection({ tag, title, description, events, cta }: Props) {
  return (
    <>
      ...
      {/* The last odd card spans both columns so a 3-item list leaves no hole. */}
      <div className="grid gap-4 lg:grid-cols-2 lg:[&>*:last-child:nth-child(odd)]:col-span-2">
        {events.map((event, i) => (
          <FadeIn key={event.title} delay={150 + i * 120}>
            <a
              ...
              className="group flex h-full items-center gap-5 rounded-[14px] border border-[#E4DFD2] bg-white p-5 transition-all duration-200 hover:translate-x-1 hover:border-orange"
            >
              ...
              <ArrowRight className="h-4 w-4 flex-none text-[#C9C2B0] transition-colors group-hover:text-orange" />
            </a>
          </FadeIn>
        ))}
      </div>
